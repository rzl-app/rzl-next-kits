...
async function getRequestURL(callingExpression: string): Promise<URL> {
  ...
  throw new Error(
    FLAG_MESSAGE(
      `Function \`${callingExpression}\` could not access the request URL. Probably you should report this as a bug. GitHub: https://github.com/rzl-app/rzl-next-kits/issues`
    )
  );
}
